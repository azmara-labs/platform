...
description: Azmara is built around a single idea — your data layer should not be an external dependency, it should be the foundation.
---
...
await db.createTable("orders", { customer: "string", total: "number", paid: "boolean" });

// 2. Query it
const unpaid = query(await db.getAll("orders"))
  .where(o => !o.paid)
